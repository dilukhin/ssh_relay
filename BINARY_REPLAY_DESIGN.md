...
Статус: **принят в PR #42; реализация ведётся отдельно по Issue #44**.
...
Истёкшая запись недоступна и по UUID, и через `--last`, даже если физическое удаление не удалось. Оставшиеся файлы и запись продолжают учитываться в byte/count budget и повторной очистке. При частичном удалении metadata удаляется последним, чтобы не потерять terminal timestamp, пока остаётся raw.
...
Весь цикл `gc.lock` — проверка/восстановление, создание, защищённый участок и освобождение — дополнительно сериализуется OS lock на постоянном приватном `replay/v1/gc.guard`. POSIX использует `flock(LOCK_EX|LOCK_NB)`, Windows — `msvcrt.locking(LK_NBLCK, 1)` на диапазоне от offset 0 (допустим за EOF). Descriptor открыт до конца цикла; закрытие/смерть процесса освобождает OS lock. При занятости или ошибке OS lock операция отклоняется без перехода к незащищённому протоколу.

`gc.guard` создаётся с `O_EXCL` либо открывается с обычными проверками path/ACL/inode; никогда не удаляется и не заменяется участниками/GC. Поэтому все участники блокируют один файловый объект, а другой участник не может заменить `gc.lock` между `lstat` и `unlink`. Повторный `lstat` сам по себе этой гарантии не даёт. Требуется локальная файловая система с рабочими OS locks; одновременная работа с предварительными версиями replay, не использующими guard, не поддерживается. При обновлении все daemon нужно перезапустить.
...
Явный `replay --request-id` под тем же storage lock проверяет только адресованный UUID-dir, его путь, права, все entries, metadata, TTL и raw integrity. Посторонние каталоги не сканируются и не меняются. Полное сканирование для `--last`, GC и reservation остаётся консервативным: неизвестный/небезопасный объект останавливает операцию. Исключение для corrupt metadata из §13.4 сохраняется только для явного UUID и всегда даёт partial/unknown.
