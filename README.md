...
Основной сценарий: пользователь один раз вручную запускает `daemon`, проходя SSH-аутентификацию паролем либо заданным ключом/сертификатом, после чего CLI-агент, в частности OpenCode, выполняет удалённые команды через локальный вызов `exec`, не используя прямой SSH и не запрашивая учётные данные повторно.
...
```text
py .\ssh_relay.py daemon --host HOST --user USER [--port PORT] [-i PATH] [--ask-key-passphrase] [--known-hosts PATH] [--command-timeout SECONDS]
...
Устанавливает SSH-соединение и запускает локальный relay. Без параметра `--identity-file` используется прежний режим: SSH-пароль вводится вручную и не сохраняется.
...
При необходимости вместо этого сохраняется вход по паролю без параметра `-i`.
...
* SSH-пароль или passphrase ключа вводятся только пользователем в терминале daemon и не сохраняются на диск.
...
Вход по ключу в другом запуске daemon:
...
```cmd
cmd /V:ON /C "py .\ssh_relay.py exec ^"sh -c 'exit 7'^" & echo Exit code: !ERRORLEVEL!"
```
