...
Справка `--help` и ошибки разбора аргументов выводятся без временных меток, с исходными отступами argparse. Метки времени включаются после успешного разбора аргументов только для диагностики daemon, включая запуск через `--detach`.

`--detach` доступен только с `--identity-file` без интерактивного passphrase и без `--enable-sudo`.

После обновления daemon старую активную сессию нужно остановить и запустить заново. Клиент `0.10.0` откажется запускать протокол длительных `upload`/`download` через daemon старее `0.8.0`, чтобы старый daemon не интерпретировал служебные чанки как обычную передачу. Перед risky-командой клиент также требует capability safe receipt v1; старый daemon без capability получает только read-only `status`, а пользовательская risky-команда не отправляется.
...
## Локальный двоичный replay (0.10.0)

Короткие `exec` и `sudo-exec` сохраняют исходные байты stdout/stderr до декодирования
в приватный временный буфер. Команда `replay` позволяет выбрать другую кодировку
**без повторного выполнения удалённой команды** и работает после остановки daemon.

Пример для cmd.exe и PowerShell (синтаксис одинаков):

```text
ssh_relay exec --name prod --request-id 3d33f5d8-9b2e-4f5b-8ad7-1b23e7142860 "hostname"
ssh_relay replay --name prod --request-id 3d33f5d8-9b2e-4f5b-8ad7-1b23e7142860 --encoding cp866
ssh_relay replay --name prod --last --encoding utf-8 --json
```

Для каждой новой команды используйте новый UUIDv4. Без `--request-id` клиент создаёт
его автоматически; `exec --json` возвращает `request_id`, `replay_status` и
`replay_truncated`. В обычном текстовом режиме служебные строки не добавляются.
`--last` выбирает только однозначную запись своей сессии, учитывая процессы-владельцы;
при нескольких подходящих записях нужен явный идентификатор.

`replay` строго декодирует оба потока до их вывода. Код завершения относится к
локальному чтению: `0` — полный replay, `2` — частичный/усечённый/неизвестный исход,
`1` — ошибка чтения, прав или кодировки. Исходный удалённый код доступен в JSON как
`source_command_exit_code`; он не становится кодом завершения replay.

Лимиты: последние 4 МиБ каждого потока, резерв 8 МиБ на запрос, 16 МиБ на сессию,
64 МиБ суммарно; максимум 8 записей на сессию и 32 суммарно. Реализация консервативно
учитывает активные записи также в лимитах количества. Чистый вывод хранится 5 минут,
подозрительный или неизвестный — 30 минут. Чтение не сдвигает время завершения.
Переход к другой кодировке переводит ещё живую запись в подозрительную.
Сборка мусора выполняется при обращениях и раз в минуту работающим daemon.

Буфер может содержать чувствительный **вывод команды**. Для таких вызовов используйте
`--no-replay`; команда, пароли аутентификации и session token в метаданные не записываются.
Приватные права обязательны: POSIX 0700/0600; на Windows проверяется ACL, ссылки и
reparse points отклоняются. При невозможности безопасного хранения команда выполняется
с `replay_status=unavailable`, без резервного незащищённого каталога.
Повреждённые записи или неподтверждённый владелец блокировки могут сделать буфер
недоступным: принудительного удаления неизвестных файлов нет.

После обновления остановите и заново запустите daemon. Старый daemon продолжает
выполнять команды нового клиента, но replay для них недоступен. Jobs, передачи файлов
и внутренние команды квитанций не участвуют в replay. Наличие сохранённого вывода
не разрешает повторять команды с исходом `unknown` или `partial_success`.
